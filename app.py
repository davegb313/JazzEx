...
@app.route("/jazz_styles")
def jazz_styles(username):
	profile = find_profile(username)

	jazz_styles = []
	for style in styles:
		if style['style_id'] == profile['id']:
			jazz_styles.append(style)
	return render_template('jazz_styles.html', profile=profile, styles=jazz_styles)